...
const tagsSeparator = string => {
  ...
  if (separators !== null && separators !== void 0 && separators.length) {
    const reg = /\W/;
  ...
  if (!separators || separators.length > 1 || separators.length === 0) {
    ...
    const regSeparatorCandidates = /\W/g; // Capturing special characers- these are the candidates for the separator (with dupicates). This will be used if no seperators are being passed or if an empty separators array is being passed

    let specialChars = [...string.matchAll(regSeparatorCandidates)].map(item => item[0]);

    if ((separators === null || separators === void 0 ? void 0 : separators.length) > 1) {
    ...
    inferredSeparator = (_countsArr$ = countsArr[0]) === null || _countsArr$ === void 0 ? void 0 : _countsArr$[0];
...
const numberFormatter = number => {
  ...
  let isNegative = false; // turn positive in case negative to streamline 'numberFormatter' function use
  ...
  let processedNumber = number,
      unitSuffix; //if the number got floating point fixed the number accordingly
  ...
  const [left, right] = processedNumber.split('.'); //returns the handled number seperated by commas, attach the right side if exist and append the letter representing the thousends sliced

  const obj = {
    success: true,
    message: 'Successfully formatted number',
    data: {
      number: (isNegative ? '-' : '') + left.replace(/\B(?=(\d{3})+(?!\d))/g, ',') + (right ? '.' + right : '') + ((_unitSuffix = unitSuffix) !== null && _unitSuffix !== void 0 ? _unitSuffix : '')
    }
  };
...
const phoneNumberFormatter = number => {
  ...
  const cleanNumber = number.replace(/[^0-9]/g, ''); //tests if the phone number input doesn't contain letters and also accepts hyphens, whitespace and parenthesis in specific locations.
  ...
  if (cleanNumber.length >= 7 && cleanNumber.length <= 15) {
    let formattedNumber = '';
    let count = 0; // add to the clean number hyphens by the format param
