...
const passwordValidation = password => {
  ...
  if (isValid.success) {
    let validation = [config.password.characterLen !== undefined && config.password.characterLen !== 0 ? {
      title: 'CharacterLen',
      valid: false,
      re: new RegExp('^.{' + config.password.characterLen + ',}$')
    } : null, config.password.upperCase !== undefined && config.password.upperCase !== 0 ? {
      title: 'UpperCase',
      valid: false,
      re: new RegExp('^(.*?[A-Z]){' + config.password.upperCase + ',}')
    } : null, config.password.lowerCase !== undefined && config.password.lowerCase !== 0 ? {
      title: 'LowerCase',
      valid: false,
      re: new RegExp('^(.*?[a-z]){' + config.password.lowerCase + ',}')
    } : null, config.password.num !== undefined && config.password.num !== 0 ? {
      title: 'Number',
      valid: false,
      re: new RegExp('^(.*?[0-9]){' + config.password.num + ',}')
    } : null, config.password.symbol !== undefined && config.password.symbol !== '' && config.password.symbol !== 0 ? {
...
const emailDomainValidator = email => {
  ...
  if (regex.test(email)) {
    const domainList = config.emailDomainValidator.domainList ? config.emailDomainValidator.domainList : undefined; // test if user inserted domain list
