...
const passwordValidation = (password) => {
  ...
  if (isValid.success) {
    let validation = [
      config.password.characterLen !== undefined &&
      config.password.characterLen !== 0
        ? {
            title: "CharacterLen",
            valid: false,
      ...
      config.password.num != undefined && config.password.num != 0
        ...
        : null,
      config.password.symbol !== undefined &&
      config.password.symbol !== "" &&
      config.password.symbol !== 0
        ? {
    ...
    validation = validation.filter(
      (validator) => validator !== null && validator !== undefined
    );
...
const passwordStrength = (
  password,
  options = config.password.strengthOptions,
  allowedSymbols = config.password.symbol
) => {
